        ...
        private int CheckPress()
        {
            if (Mouse.GetState().LeftButton == ButtonState.Pressed)
            {
                if (!tapped)
                {
                    ...
                    if (new Rectangle(new Point(863, 520), new Point(components[(int)TextureSQuit.yes].Width, components[(int)TextureSQuit.yes].Height)).Contains(Mouse.GetState().X, Mouse.GetState().Y)) tapped = true;
                }
                tapped = true;
            }
            else tapped = false;
